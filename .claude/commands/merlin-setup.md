...
Merlin recognizes seven business categories. They drive setup routing, default platforms, offering nouns, revenue source, and prompt wording. **Detect from the website, then CONFIRM with one chip-style question** — don't assume. Never guess silently for non-ecommerce brands; a wrong vertical makes the entire session speak the wrong language.
...
**How to pick:**
1. Auto-detect from signals: `.myshopify.com` redirect, presence of `/products.json`, `/pricing`, `/app.*`, Stripe checkout, App Store/Play Store badge, domain keywords (`.games`, `app.*`, `get-*`).
2. If high confidence → state your pick in one line ("Looks like a SaaS — users sign up for a plan, right?") and let the user correct if wrong.
3. If low / no confidence → ask ONCE with the seven canonical options, written in plain English: "Which best describes your business? (eCommerce / SaaS / Games / Creator / Local Services / Agency / B2B)".
4. Write the canonical key (`ecommerce`, `saas`, `games`, `creator`, `local`, `agency`, `b2b`) to `brand.md` as `Vertical: <key>` AND to `merlin-config.json` via the `vertical` field. Both matter: brand.md is portable, config.json is read by the Go binary at runtime.
