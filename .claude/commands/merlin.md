...
**RULES:**
- **NEVER substitute a model the user didn't ask for.** This is absolute. If the user asks for Seedance, use Seedance. If Seedance fails, rate-limits, or isn't mapped — STOP. Report the failure. Ask the user what to do (retry, try a different model, give up). Never pick a different model "as a fallback" or "since X isn't available" or "let me try Y instead". The user pays for every generation. Wasting their credits on a model they didn't want is unacceptable. This applies to: Seedance ↔ Kling, Veo ↔ anything, image-to-video ↔ text-to-video, Nano Banana Pro ↔ Seedream, edit ↔ base model, every single pair. When in doubt, ask. The binary will now FAIL LOUDLY if you try a silent substitution — use that error to tell the user what happened, not to retry with a different model behind their back.
- **If you don't know the exact fal.ai slug**, use `WebFetch` on `https://fal.ai/models/{vendor}/{model}` to find it BEFORE calling the video tool. Pass the full `fal-ai/...` slug as `falModel`. Do not guess. Do not approximate. Do not use a "close enough" alternative.
...
- **Rendering performance tables.** When Claude shows campaign or ad performance in chat, every number AND every column header MUST come from a single field in the JSON output of the corresponding action. Rules, in order:
  1. **Only use the fields that exist.** For `meta-insights`, the JSON output has a `campaign_summary` block with exactly these fields per row: `campaign_name`, `category`, `spend`, `revenue`, `roas`, `purchases`, `impressions`, `clicks`, `ctr`, `cost_per_purchase`, `ad_count`. You may render any subset of these as columns. You may NOT invent columns like "Status", "Reach", "Results", or "Budget" — those are not in the output, and making them up produces fake data that looks authoritative. If the user wants Status or Budget, run the appropriate action (`meta_ads({action: "discover"})` for status, `connection_status` for account details) and quote the result.
  2. **Never show the same metric twice.** `Cost/Purchase` and `Cost/Purch` and `CPP` are the same column — pick ONE label and use it once. Duplicate columns are a sign that Claude is re-deriving a field that was already in the output.
  3. **Use the pre-computed summary verbatim.** For `meta-insights`, always quote the `campaign_summary` rows exactly as printed. Do not sum the `ads` array yourself to cross-check — the binary already did the math correctly and summing rounded values gives different totals.
  4. **Keep tables narrow.** Max 6 columns for chat display (`Campaign`, `Spend`, `Revenue`, `ROAS`, `Purchases`, `Cost/Purchase` is the canonical set). If the user asks for more, give them a second table or a bulleted list — don't pack 10+ columns across the screen.
  5. **Campaign names are authoritative.** The `campaign_name` field in the JSON is the real Meta campaign name (e.g. "Merlin - Testing"). Use it as-is. Never flatten it to "merlin" / "external" / "account" — those are internal category labels, not display names.
